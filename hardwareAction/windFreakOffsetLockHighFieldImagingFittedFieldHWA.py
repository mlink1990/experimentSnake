...
class WindFreak(hardwareAction.HardwareAction):
    ...
    def __init__(self, callbackTimeInSequence, **traitsDict):
        ...
        self.variableMappings = {"LiOffsetLockStartFreqMHz":"LiOffsetLockStartFreqMHz",
        ...
        self.hardwareActionName = 'windFreak-offsetLock-B-field-Imaging-fitted'
    ...
    def callback(self):
        ...
        try:#YOUR CALLBACK CODE SHOULD GO IN THIS TRY BLOCK!
            ...
            if state not in self.validStates:
                ...
                try:
                    fieldShiftMHz = self.highFieldImaging.imagingFrequency(fittedFieldBGauss, state, polarisation)
                    imagingStrength = self.highFieldImaging.imagingStrength(fittedFieldBGauss, state, polarisation)
            ...
            BFieldMessage = "Windfreak: Li Imaging set for |%s> state polarisation %s,  %s G=%s (calibrated) , %s MHz detuned--> %s+%s+%s=%s MHz imaging frequency. (strength=%s)" % (state,polarisation, BfieldGauss,fittedFieldBGauss, detuningMHz,resonance0GaussMHz,fieldShiftMHz,detuningMHz,endFrequencyMHz,imagingStrength)
