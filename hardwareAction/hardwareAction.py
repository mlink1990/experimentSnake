...
class HardwareAction(traits.HasTraits):
    ...
    def _enabled_changed(self):
        ...
        if self.enabled:
            self.snakeReference.mainLog.addLine("%s was just enabled. Will perform its init method now" % self.hardwareActionName,1)
            ...
            if self.snakeReference.isRunning:#only print to log if it's disabled while snake is running
                self.snakeReference.mainLog.addLine("%s was just disabled. Will perform its close method now" % self.hardwareActionName,1)
